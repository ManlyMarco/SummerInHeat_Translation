...
internal static class ReleaseTool
{
    ...
    private static void Main(string[] args)
    {
        try
        {
            ...
            if (targetPath == null)
            {
                ...
                while (!string.IsNullOrWhiteSpace(assLoc) && !assLoc.EndsWith("SummerInHeat_Translation")) //todo generalize and merge with other copies of this tool into a shared repo?
                    assLoc = Path.GetDirectoryName(assLoc);

                if (assLoc != null && assLoc.EndsWith("SummerInHeat_Translation"))
                    targetPath = assLoc;
            }
            ...
            {
                var root = new DirectoryInfo(targetPath);
        ...
        catch (Exception e)
        {
            ...
            Console.WriteLine("Aborting release creation because of an unexpected error: " + e);
        }
    ...
    private static void DoRelease(string translationName, DirectoryInfo root, int translationPriorityNumber)
    {
        ...
        foreach (var languageCode in Directory.GetDirectories(tlTopDir, "*", SearchOption.TopDirectoryOnly).Select(Path.GetFileName))
        {
            ...
            {
                if (!Directory.GetFiles(texDir, "*.png", SearchOption.AllDirectories).Any())
                    continue;
                ...
                AddToZip(texZipPath, $"BepInEx\\Translation\\{languageCode}\\Texture\\{translationPriorityNumber:00}_{translationName}_{Path.GetFileName(texDir)}.zip");
            }
            ...
            if (Directory.Exists(assetDir) && Directory.GetFiles(assetDir, "*.txt", SearchOption.AllDirectories).Any())
            {
                ...
                AddToZip(assZipPath, $"BepInEx\\Translation\\{languageCode}\\RedirectedResources\\assets\\{translationPriorityNumber:00}_{translationName}.zip");
            }
